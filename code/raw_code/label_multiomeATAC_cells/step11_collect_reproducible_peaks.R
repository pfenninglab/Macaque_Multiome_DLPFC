...
peak_rds_fn = list.files(path = here(DATADIR,'ArchRProjects', 'relabeled_DLPFC','PeakCalls'), 
...
system('mkdir -p logs')
halmapper_script = '/home/azwang2/atac_data_pipeline/scripts/halper_map_peak_orthologs.sh'
sbatch = 'sbatch -p pfen1 -w compute-1-11,compute-1-12 --mem 10G'
SOURCE_SPECIES = 'Macaca_mulatta'
target_species = '-t Homo_sapiens,Mus_musculus,Rattus_norvegicus'
...
thecall = paste(sbatch, halmapper_script,
                ...
                halLiftover)
cat(thecall, file= here(CODEDIR, 'step12_map_to_species.sh'), sep = '\n')
system(paste('chmod u+x', here(CODEDIR,'step12_map_to_species.sh')))
system('./step12_map_to_species.sh')

# # map to rat
# SOURCE_SPECIES = 'Macaca_mulatta'
# TARGET_SPECIES = c('Rattus_norvegicus')
...
#                 rep(peak_files, times = length(target_species)),
#                 halLiftover)
# cat(thecall, file= here(CODEDIR,'step12_map_to_rat.sh'), sep = '\n')
# system(paste('chmod u+x', here(CODEDIR, 'step12_map_to_rat.sh')))
# system('./step12_map_to_rat.sh')
# 
# # map to mouse
# SOURCE_SPECIES = 'Macaca_mulatta'
...
#                 rep(peak_files, times = length(target_species)),
#                 halLiftover)
# cat(thecall, file= here(CODEDIR,'step12_map_to_mouse.sh'), sep = '\n')
# system(paste('chmod u+x', here(CODEDIR, 'step12_map_to_mouse.sh')))
# system('./step12_map_to_mouse.sh')
# 
# 
